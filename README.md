# DevOps Stage 0 Challenge

## About Me
- **Name:** Osagunna Oyindamola
- **Slack Username:** @yDEEIII
- **Track:** DevOps

## Project Description
This project demonstrates the deployment of a basic NGINX web server on a cloud platform as part of the HNG13 DevOps Stage 0 challenge. It showcases fundamental server configuration and deployment skills.

## Server Details
- **Server IP/Domain:** http://your-server-ip (server will be added after deployment)
- **Status:** Live and publicly accessible

## Challenge Requirements
- Deploy NGINX web server on port 80
- Custom HTML page with deployment information
- Public accessibility via HTTP
- GitHub repository with documentation

## Deployment Platform
[Specify which platform you're using: AWS, DigitalOcean, etc.]